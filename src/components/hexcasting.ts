...
const rfaQueue: ((dt: number) => void)[] = [];
const colorCache = new Map();

function getColorRGB(ctx: CanvasRenderingContext2D, str: string) {
...
function startAngle(str: string): number {
  switch (str) {
    case "east":
      return 0;
    case "north_east":
...
function offsetAngle(str: string): number {
  switch (str) {
    case "w":
      return 0;
    case "q":
...
export function initializeElem(canvas: HTMLCanvasElement) {
  ...
  const getProp = (n: string) => style.getPropertyValue(n);
  ...
  const tick = (dt: number) => {
    ...
    const bgColors = getColorRGB(context, getProp("--color-gray-800"));
...
export function hookLoad(elem: Element) {
  const canvases = elem.querySelectorAll("canvas");
  canvases.forEach(initializeElem);
}

document.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll(".pattern-display").forEach(hookLoad);
  function tick(prevTime: number, time: number) {
...
document.querySelectorAll(".copy").forEach((elem) => {
  elem.addEventListener("click", async () => {
    await navigator.clipboard.writeText(
      (elem as HTMLButtonElement).dataset.copy!,
    );
    const template = document.getElementById(
      "noti-template",
    ) as HTMLTemplateElement;
    const noti = (template.content.cloneNode(true) as DocumentFragment)
      .children[0] as HTMLDivElement;
    noti.classList.add("transform", "ease-out", "duration-300", "transition");
    noti.classList.add(
      "translate-y-2",
      "opacity-0",
      "sm:translate-y-0",
    ...
    document.getElementById("noti-area")!.appendChild(noti);
    requestAnimationFrame(() => {
      noti.classList.remove(
        "translate-y-2",
        "opacity-0",
        "sm:translate-y-0",
        "sm:translate-x-2",
      );
      noti.classList.add("translate-y-0", "opacity-100", "sm:translate-x-0");
      noti.addEventListener(
        ...
        () => {
          ...
          const closer = () => {
            ...
            closing = true;
            noti.classList.add("transition", "ease-in", "duration-100");
          ...
          const timeout = setTimeout(closer, 1500);
          noti.addEventListener("click", closer, {
            once: true,
          });
        },
        {
